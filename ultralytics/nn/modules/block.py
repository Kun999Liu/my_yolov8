...
from ultralytics.utils.ndvi import NDSI_Layer
...
#         # self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
#         # self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
#         self.c1 = Conv(c1, c2, 1, 1)
#         self.c2 = Conv(self.c, c2, 1)
#         self.bottleneck = Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0)
#
#     def forward(self, x):
#         """Forward pass through C2f layer."""
#         b1, b2, b3, b4 = self.c1(x).chunk(4, 1)
#         y = self.bottleneck(self.bottleneck(self.bottleneck(self.bottleneck(b1) + b2) + b3) + b4)
#         y = self.c2(y)
#         return y
#         # y = list(self.cv1(x).chunk(2, 1))
#         # y.extend(m(y[-1]) for m in self.m)
...
#         return x + self.cv2(self.cv1(self.sa(x))) if self.add else self.cv2(self.cv1(self.sa(x)))
