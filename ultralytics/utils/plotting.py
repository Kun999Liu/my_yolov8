...
def plot_images(
    ...
    mosaic = np.full((int(ns * h), int(ns * w), images[0].shape[0]), 255, dtype=np.uint8)
    for i in range(bs):
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/exp")):
    ...
    Args:
        ...
        save_dir (Path, optional): Directory to save results. Defaults to Path('runs/exp').
    """
...
# # 按照真实通道数输出
# def feature_visualization_all(x, module_type, stage, n=32, save_dir=Path("runs/exp")):
#
#     for m in {"Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder"}:  # all model heads
#         if m in module_type:
#             return
...
#             blocks = torch.chunk(x[0].cpu(), channels, dim=0)  # select batch index 0, block by channels
#             n = min(n, channels)  # number of plots
#             _, ax = plt.subplots(math.ceil(n / 12), 12, tight_layout=True)  # 8 rows x n/8 cols
#             ax = ax.ravel()
#             plt.subplots_adjust(wspace=0.05, hspace=0.05)
#             for i in range(n):
#                 ax[i].imshow(blocks[i].squeeze())  # cmap='gray'
...
# # 按照真实尺寸输出
# def feature_visualization_all(x, module_type, stage, save_dir=Path("runs/exp")):
#     for m in {"Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder"}:  # all model heads
#         if m in module_type:
#             return
#     if isinstance(x, torch.Tensor):
